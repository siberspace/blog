<script lang="ts">
	...
	onMount(() => {
		...
		// Use the visual viewport height if available (most accurate on mobile),
		// otherwise fall back to window.innerHeight
		const vh = window.visualViewport?.height ?? window.innerHeight;
		viewportEl.style.height = `${vh}px`;

		// Prevent subsequent visual-viewport resizes (address-bar hide/show) from
		// reflowing the landing section.  We intentionally do NOT update the height.
		// Also stop Brave / Chromium from recalculating layout during scroll by
		// keeping the height fixed to the initial value.
	});
...
<svelte:head>
	...
	<meta property="og:url" content="https://siberrr.space/printpress" />
...
<style>
	...
	.landing__viewport {
		...
		z-index: 3;
		overflow: clip;                    /* prevent scroll bleed from children */
		-webkit-transform: translateZ(0);  /* promote to own GPU layer on mobile */
